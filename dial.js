...
function refreshCount() {
    let targetCount = localStorage.getItem("target"); //target -> 목표 횟수
    let tmCountIndex = localStorage.getItem("dialCount"); //current count
    targetCount = Number(targetCount);
    tmCountIndex = Number(tmCountIndex);
    let countIndex = tmCountIndex / targetCount;
    countIndex = countIndex * 100;
    dial.value = countIndex;
    dial.animateStart();
}
...
var dial = new Dial(containers[0]);
function refreshSet() {
    let targetSet = localStorage.getItem("set"); //set -> 목표세트
    let tmSetIndex = localStorage.getItem("dialSet"); //current set
    targetSet = Number(targetSet);
    tmSetIndex = Number(tmSetIndex);
